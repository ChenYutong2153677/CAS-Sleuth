...
<!-- <div class="background">
    ...
    <div>
        <el-card class="card-head">
            ...
            <template #dropdown>
                <el-dropdown-menu>
                    <el-dropdown-item v-for="(v,index) in virus_accession_list" :key="v.order" style="font-size:small;width: 80px;height: 10px;color:#384FB9" @click="goto_virus(index)"> {{ v }}</el-dropdown-item>
                </el-dropdown-menu>
            </template>
            </el-dropdown>
            <el-button class="left-button" @click="left_arrow" border></el-button>
            <div style="position: absolute;left:89%;top:43%;background-color: #ffffff;width:3%;border-radius: 20px;text-align: center;" >{{ number }}</div>
            <el-button class="right-button" @click="right_arrow" border></el-button>
            
        </el-card>
        <div style="z-index: 9;position: absolute;height: 100%;">
                <el-checkbox label="1" style="position: absolute;top:27%;left:210px;width:150px;height:50px;background-color:#00ACC6;border-radius: 30px;color:white;border: 0ch;" border>Basic</el-checkbox>
                <el-checkbox label="2" class="grna_info" v-model="ischecked" border>grna_info</el-checkbox>
        ...
        <el-card class="card-content-bg">
            <el-card style="position: absolute;left:5%; background-color: white;width: 90%;height: 500px;opacity: 1;">
                ...
                <button class="button-cas9" :style="{'z-index':cas9,'background-color':cas9_color}" @click="cas_9_click">cas 9</button>
                <button class="button-cas12" :style="{'z-index':cas12,'background-color':cas12_color}" @click="cas_12_click">cas 12</button>
                <button class="button-cas13" :style="{'z-index':cas13,'background-color':cas13_color}" @click="cas_13_click">cas 13</button>
            <el-card style="position: absolute;left:5%;top:22% ;background-color: white;width: 90%;height: 80%;opacity: 1;z-index:8">
                ...
                <div style="position:absolute;background: linear-gradient(#EEEEEE, #EDEDED00);top:5%;left:10%;width:80%;height: 80%;border-radius: 60px;z-index:8">
                    <div style="position: absolute;background-color: #5EC7D8;width: 50%;height: 50px; border-radius: 20px;top:5%;left:23%;z-index: 15;">
                        <el-text style="position: absolute; top: 30%;left: 42%;font-family: 'Arial';color: white;font-size: large;">sequence</el-text>
                    </div>
                    <div style="position: absolute;background: linear-gradient(#FFFFFF, #EDEDED00);top:6%;left:5%;width:90%;height: 70%;border-radius: 30px;">
                        ...
                        <div style="position:absolute;top: 20%;left: 5%; border-top:5px solid #5CD2C7 ;width: 90%;height: 5px;">
                            <div v-for="(v,index) in cas_result_order_by_percentage" :key="v.index" :style="{left:v.percentage*700 + 'px'}" class="spacer-div">
                                <div v-if="index%2==0">
                                    <el-button style="background-color:#6FD7CE ;border-radius: 20px;color:black;position:absolute;top:-65px;left:-17px;height: 20px;width: 40px;font-size: 5px;" @click="go_to_below(v.index)" :id="'button'+v.index">{{ v.index }}</el-button>
                    ...
                    <div style="position: absolute;background-color: #5EC7D8;width: 50%;height: 50px; border-radius: 20px;top:41%;left:23%;z-index: 15;">
                        <el-text style="position: absolute; top: 30%;left: 42%;font-family: 'Arial';color: white;font-size: large;">grna_info</el-text>
                    </div>
                    <div style="position: absolute;background: linear-gradient(#FFFFFF, #EDEDED00);top:42%;left:5%;width:90%;height: 60%;border-radius: 30px;">
                        ...
                        <div style="position: absolute;width: 90%;height: 5%;background-color: rgba(94, 199, 216, 1);left:5%;border-radius:30px ;z-index:30;top:10%">
                        ...
                        <el-text style="position: absolute;color: white;left:85%;top:40%">Index</el-text>
                            ...
                            <div  v-for="v in cas_result_order" :key="v.index" class="sequence" :style="{opacity:1.2-v.order/10}" :id="v.index">
...
//    const router=useRouter()
   ...
   onMounted(()=>{
    ...
    await axios.get('/api/virus/detail?virus_id='+virus_id.value)
      .then(response => {
        // 处理成功响应
     ...
     const get_cas_information=async ()=>{
      ...
      await axios.get('/api/cas/findAllSeq?type='+type.value+'&virus_id='+virus_id.value)
      .then(response => {
        // 处理成功响应
      ...
      function go_to_below(index){
        ...
        const height=document.getElementById(index).offsetTop+document.documentElement.clientHeight*1.5
        console.log(height)
